...
class Ultimatenews_Content_Widget_Base extends Engine_Content_Widget_Abstract
{
    ...
    public function indexAction()
    {
        ...
        $this->setScriptPath('application/modules/Ultimatenews/widgets/top-ultimatenews');
    }
    ...
    protected function _getShortDescription($item)
    {
        ...
        if ($des != null)
        {
            $result = (strlen($des) > 1000) ? ($this->view->string()->chunk(substr($des, 0, 125), 125) . ' ...') : $des;
        }
        else
        {
            $result = (strlen($str) > 1000) ? $this->view->string()->chunk(substr($str, 0, 125), 125) . ' ...' : $str;
        }
        return $result;
    }
    ...
    protected function _getShortImage($item)
    {
        if ($item->image != "")
        {
            return '<img class="ynultimatenews-smallthumb" src="' . $item->image . '" align="left" />';
        }
    }
