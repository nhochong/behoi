...
<ul class="experience-mode-views clearfix" id="experience-content-mode-views-<?php echo $this->identity?>">
  <?php 
  	foreach( $this->experiences as $item )
    {
		...
		{
			echo $this->partial('_listItem.tpl', 'experience', array('item' => $item, 'type' => 'top'));
			echo $this->partial('_gridItem.tpl', 'experience', array('item' => $item, 'type' => 'top'));
		}
	}
  	?>
    <?php if(count($this->experiences) == $this->limit): ?>
        <li>
          <div class="experience-more clearfix">
              <a href="<?php echo $this->url(array(),'default'); ?>experiences/listing/orderby/creation_date" >
