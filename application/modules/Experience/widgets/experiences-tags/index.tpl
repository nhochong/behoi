...
<ul class="global_form_box" style="margin-bottom: 15px;">
    <span id="script" style="font-size: 9pt;">
        ...
        <span style="<?php if($tag->count > 99 && $tag->count < 599): echo " font-size:".($tag->count/80 + 8)."pt"; elseif($tag->count > 599): echo "font-size: 14pt"; endif; ?>">
          <a href='javascript:void(0);' onclick='javascript:tagAction(<?php echo $tag->tag_id; ?>);'><?php echo $tag->
...
<script type="text/javascript">
    ...
    {
        if($('experience_filter_form')) {
            $('tag').value = tag;
            $('experience_filter_form').submit();
        }
        else
        {
            window.location = "<?php echo $this -> url(array('action' => 'listing'), 'experience_general')?>" + "?tag=" + tag;
        }
    }
