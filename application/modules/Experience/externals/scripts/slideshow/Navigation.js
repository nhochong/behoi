...
document.addEvent('domready', function(){
	...
	var navs = $('experience_pagination').getElements('a');

	// create a basic slideshow
	navSlideShow = new SlideShow('experience_navigation-slideshow', {
		autoplay: true,
		delay: 5000,
	...
	if($$('.experience_newsalbums').length > 0) {
		var itemWidth =  parseInt($$('.experience_newsalbums')[0].getStyle('width'));
		...
		$('experience_navigation-slideshow').style.width = slideWidth + "px";
		console.log(itemWidth);
		if($$('.experience_newsalbums').length >= 4) {
			$$('.experience_newsalbums').each(function(item, index){
