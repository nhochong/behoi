...
class Ynresponsivepurity_Widget_EventGridController extends Engine_Content_Widget_Abstract
{
    public function indexAction()
    {
        ...
        if (Engine_Api::_()->hasModuleBootstrap('ynlocationbased') && Engine_Api::_() -> hasModuleBootstrap('ynevent')) {
            $select = Engine_Api::_()->ynlocationbased()->getLocationBasedSelect('ynevent', 'events');
        }
        else
        {
            $select = $table->select();
