...
class Ynlistings_IndexController extends Core_Controller_Action_Standard
{
    ...
    public function viewAction()
    {
        ...
        $photos->setItemCountPerPage(100);
        //get videos
        $enabledVideoType = Engine_Api::_()->ynlistings()->getListingVideoType();
        if ($enabledVideoType) {
            $videoFunctions = array(
                'video' => 'getVideosPaginator',
                'ynvideo' => 'getVideosPaginator',
                'ynultimatevideo_video' => 'getUltimateVideosPaginator'
            );
            $tableMappings = Engine_Api::_()->getItemTable('ynlistings_mapping');
            $params['listing_id'] = $subject->getIdentity();
            $this->view->videos = $videos = $tableMappings->$videoFunctions[$enabledVideoType]($params);
            $videos->setCurrentPageNumber(1);
            $videos->setItemCountPerPage(100);
        }
    ...
    public function editAction()
    {
        ...
        try {
            ...
            if (!empty($values['toValues'])) {
                $videoItem = Engine_Api::_()->getItemByGuid($values['toValues']);
                $listing->video_id = $videoItem->getIdentity();
                $listing->video_type = $videoItem->getType();
            }
            if ($values['is_end'] == '1') {
            ...
            if (!empty($values['toValues'])) {
                ...
                $hasItem = $tableMappings->checkHasItem($listing->getIdentity(), $videoItem->getIdentity(), 'profile_' . $videoItem->getType());
                if (!$hasItem) {
                    $row = $tableMappings->createRow();
                    $row->setFromArray(array(
                        'listing_id' => $listing->getIdentity(),
                        'item_id' => $videoItem->getIdentity(),
                        'user_id' => $viewer->getIdentity(),
                        'type' => 'profile_' . $videoItem->getType(),
