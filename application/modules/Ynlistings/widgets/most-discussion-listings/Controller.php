...
class Ynlistings_Widget_MostDiscussionListingsController extends Engine_Content_Widget_Abstract
{
    public function indexAction()
    {
        ...
        if (Engine_Api::_()->hasModuleBootstrap('ynlocationbased')) {
            $select = Engine_Api::_()->ynlocationbased()->getLocationBasedSelect('ynlistings', 'listings', array("COUNT($postTblName.post_id) as discuss_count"));
        }
        else {
            $select = $listingTable->select()->from("$listingTblName", array("$listingTblName.*", "COUNT($postTblName.post_id) as discuss_count"));
        }
        $select
            ...
            ->group("$listingTblName.listing_id")->order("COUNT($postTblName.post_id) DESC")
