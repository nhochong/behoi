...
class Ynlistings_Widget_ListMostItemsController extends Engine_Content_Widget_Abstract
{
    public function indexAction()
    {
        ...
        if ($recentType == 'creation') {
            $recentSelect->order('listing_id DESC');
        } else {
            $recentSelect->order($recentCol . ' DESC');
        }
        $this->view->recentListings = $listings = $table->fetchAll($recentSelect);
        $count = $itemCount;
        ...
        $popularSelect
            ...
            ->limit($count);
        $this->view->popularListings = $popularListings = $table->fetchAll($popularSelect);
    }
