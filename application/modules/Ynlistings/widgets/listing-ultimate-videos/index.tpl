...
<script type="text/javascript">
    en4.core.runonce.add(function(){
        <?php if (!$this->renderOne): ?>
            ...
            $('ynlistings_ultimatevideos_previous').style.display = '<?php echo ( $this->paginator->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>';
            $('ynlistings_ultimatevideos_next').style.display = '<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() ? 'none' : '' ) ?>';

            $('ynlistings_ultimatevideos_previous').removeEvents('click').addEvent('click', function(){
            ...
            $('ynlistings_ultimatevideos_next').removeEvents('click').addEvent('click', function(){
                en4.core.request.send(new Request.HTML({
                    url : en4.core.baseUrl + 'widget/index/content_id/' + <?php echo sprintf('%d', $this->identity) ?>,
                    data : {
                        format : 'html',
...
<?php if(count($this->paginator)>0):?>
    ...
    <div class="clearfix">
        <div id="ynlistings_ultimatevideos_previous" class="paginator_previous">
        	<?php echo $this->htmlLink('javascript:void(0);', $this->translate('Previous'), array(
        	  'onclick' => '',
        	  'class' => 'buttonlink icon_previous'
        	)); ?>
        </div>
        <div id="ynlistings_ultimatevideos_next" class="paginator_next">
