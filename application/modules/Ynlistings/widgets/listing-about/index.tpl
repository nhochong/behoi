		...
		<div class="ynlisting_btn_follow">
			<?php if($this -> viewer -> getIdentity()):?>
				<span class='<?php echo ($this->isFollowed)? "unfollow_seller" : "follow_seller" ?>' id='follow_seller'>
					...
					<span class="label">
					<?php echo ($this->isFollowed) ? $this->translate('Unfollow') : $this->translate('Follow'); ?>		
					</span>
				</span>
			<?php endif;?>
		</div>
	      ...
	      new Request.JSON({
				...
				onSuccess: function(responseJSON) 
				{
					if(responseJSON.json == 'true')
					{
						...
						$('follow_seller').getElement('span.label').set('text', '<?php echo $this->translate('Unfollow')?>');
					}
					else
					{
						$('follow_seller').removeProperty('class');
						$('follow_seller').addClass('follow_seller');
						$('follow_seller').getElement('span.label').set('text', '<?php echo $this->translate('Follow')?>');
					}
				}
		  }).send();
      });
