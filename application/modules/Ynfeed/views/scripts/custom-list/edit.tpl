...
?></b>
	 	...
	 	<div class="ynfeed_popup_options_right">
	  	<input type='text' class='ynfeed_popup_searchbox text suggested' name='search' id='field_search' size='20' maxlength='100' alt='<?php echo $this->translate('Search') ?>' onkeyup="getContentItem(1)" />
	  </div>  
	</div>
	...
	<div class="popup_btm">
		...
		<form method="post" action="" id="form_custom_list">
			...
			<div class="ynfeed_feed_popup_bottom" id="buttons-element">
				<button type='button' onClick='submitListForm()'>
                  <?php echo sprintf($this->translate('Edit list with %s items'),'<span id="item_count">0</span>'); ?> </button>
				<?php $session = new Zend_Session_Namespace('mobile');
				$isMobile = $session -> mobile;?>
				<button href="javascript:void(0);" onclick="<?php if($isMobile) echo 'history.go(-1); return false;'; else echo 'javascript:parent.Smoothbox.close()'?>"><?php echo $this->translate("Cancel"); ?></button>&nbsp;&nbsp;&nbsp;<?php echo $this->translate("or"); ?>&nbsp;
				<a href="<?php echo $this->url(array('controller'=>'custom-list','action' => 'delete','list_id'=>$this->list->list_id),'ynfeed_extended') ?>" ><?php echo $this->translate("delete") ?></a>
			</div>
		</form>
	</div>	
</div>
