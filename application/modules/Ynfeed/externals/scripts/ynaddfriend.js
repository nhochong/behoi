var YnAddFriendAutocompleter = new Class({
	...
	initialize : function(element, options) {
		...
		this.build();
		this.observer = new YnAddFriendObserver(this.element, this.prefetch.bind(this), $merge({
	...
	choiceSelect : function(choice) {
		...
		if (this.options.autocompleteType == 'message') {
			...
			if (token.friends) 
			{
				...
				this.doAddValueToHidden(token.label, friend_ids, 'ynfeed_friendValues', true, ' tag_friend');
			} else
				this.doAddValueToHidden(choice.id, token.id, 'ynfeed_friendValues', true);
		}
	...
	doAddValueToHidden : function(name, toID, hideLoc, newItem, list) 
	{
		...
		if (this.checkSpanExists(name, toID)) 
		{
			if (previousToValues == '') 
			{
				...
				document.getElementById('ynfeed_mdash').innerHTML = ' — ' + langs['with'];
				document.getElementById('ynfeed_dot').innerHTML = '.';
			} 
			else 
			{
			...
			if($('add-friend-button'))
				if(!$('add-friend-button').hasClass('addfriend_active'))
					$('add-friend-button').addClass('addfriend_active');
			friend_tagged.push(name);
			this.doPushSpan(name, toID, newItem, hideLoc, list);
			this.doPushWithFriend();
		}
	},
	doPushWithFriend: function()
	{
	...
	doPushSpan : function(name, toID, newItem, hideLoc, list) {
		...
		if (newItem) {
			...
			myElement.innerHTML = name + " <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\"" + toID + "\", \"" + name  + "\", \"" + hideLoc + "\");'>x</a>";
		} else {
			myElement.id = "tospan_" + name + "_" + toID;
			myElement.innerHTML = name + " <a href='javascript:void(0);' onclick='this.parentNode.destroy();removeFromToValue(\"" + toID + "\", \"" + name  + "\", \"" + hideLoc + "\");'>x</a>";
		}
		...
		if (list == null)
			list = "";
		myElement.addClass("ynfeed_tag" + list);

		document.getElementById('ynfeed_friendValues_element').appendChild(myElement);
		this.fireEvent('push');
	},
...
YnAddFriendAutocompleter.Base = YnAddFriendAutocompleter;
...
YnAddFriendAutocompleter.Local = new Class({

	Extends : YnAddFriendAutocompleter,
 ...
 * @copyright	Author
 */
var YnAddFriendObserver = new Class({
...
YnAddFriendAutocompleter.Request = new Class({
	...
	query : function() {
		...
		data['users'] = $('ynfeed_friendValues').value;
...
YnAddFriendAutocompleter.Request.JSON = new Class({

	Extends : YnAddFriendAutocompleter.Request,
...
YnAddFriendAutocompleter.Request.HTML = new Class({
...
YnAddFriendAutocompleter.Ajax = {
	Base : YnAddFriendAutocompleter.Request,
	Json : YnAddFriendAutocompleter.Request.JSON,
	Xhtml : YnAddFriendAutocompleter.Request.HTML
};
