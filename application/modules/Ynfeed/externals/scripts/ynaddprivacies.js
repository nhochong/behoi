var YnPrivacyAutocompleter = new Class({
	...
	initialize : function(element, options) {
		...
		this.build();
		this.observer = new YnPrivacyObserver(this.element, this.prefetch.bind(this), $merge({
	...
	build : function() {
		if ($(this.options.customChoices)) 
		{
			...
			this.choices.setStyle('z-index', this.options.zIndex);
		...
		$('ynfeed_Privacy').addEvent('click', this.onClick.create({bind : this,
				arguments : false,
				delay : 100}));
		this.element.addEvent('keydown', this.onKeyDown.create({bind : this,
				arguments : false,
				delay : 100}));
	},
	onClick : function()
	{
		...
		if(query == '')
		{
			...
			this.queryIndex = 0;
			if(this.cachedQueryValue == undefined)
			{
				this.query();
			}
	...
	choiceSelect : function(choice) {
		...
		if (this.options.autocompleteType == 'message') 
		{
			...
			switch(token.type)
      		{
      			case 'general':
      				name = 'ynfeed_GEValues';
      			break;
      			case 'network':
      				name = 'ynfeed_NEValues';
      			break;
      			case 'friendlist':
      				name = 'ynfeed_FLValues';
      			break;
      			case 'user':
      				name = 'ynfeed_FRValues';
      			break;
      			case 'group':
      				name = 'ynfeed_GRValues';
      			break;
      		}
			this.doAddValueToHidden(choice.id, token.id, name, true, token.type);
		}

	},

	doAddValueToHidden : function(name, toID, hideLoc, newItem, type) 
	{
		...
		if (this.checkSpanExists(name, toID)) 
		{
			...
			this.doPushSpan(name, toID, newItem, hideLoc, type);
		}
	},
	doPushSpan : function(name, toID, newItem, hideLoc, type) {
		// summary:
		// This is called to create a new span item with the id.

		var myElement = new Element("span");
		if (newItem) {
			myElement.id = "tospan_privacy_" + name + "_" + toID;
			myElement.innerHTML = name + " <a href='javascript:void(0);' onclick='this.parentNode.destroy();removePrivacyFromToValue(\"" + toID + "\", \"" + name  + "\", \"" + hideLoc + "\");'>x</a>";
		} else {
			myElement.id = "tospan_privacy_" + name + "_" + toID;
			myElement.innerHTML = name + " <a href='javascript:void(0);' onclick='this.parentNode.destroy();removePrivacyFromToValue(\"" + toID + "\", \"" + name  + "\", \"" + hideLoc + "\");'>x</a>";
		}

		myElement.addClass("ynfeed_tag yfprivacy_tag_" + type);

		document.getElementById('ynfeed_privacyValues_element').appendChild(myElement);
		this.fireEvent('push');
	},
	...
	checkSpanExists : function(name, toID) {
		var span_id = "tospan_privacy_" + name + "_" + toID;
		if ($(span_id)) {
			return false;
		} else
			return true;
...
YnPrivacyAutocompleter.Base = YnPrivacyAutocompleter;
...
YnPrivacyAutocompleter.Local = new Class({

	Extends : YnPrivacyAutocompleter,
 ...
 * @copyright	Author
 */
var YnPrivacyObserver = new Class({
...
YnPrivacyAutocompleter.Request = new Class({
...
YnPrivacyAutocompleter.Request.JSON = new Class({

	Extends : YnPrivacyAutocompleter.Request,
...
YnPrivacyAutocompleter.Request.HTML = new Class({
...
YnPrivacyAutocompleter.Ajax = {
	Base : YnPrivacyAutocompleter.Request,
	Json : YnPrivacyAutocompleter.Request.JSON,
	Xhtml : YnPrivacyAutocompleter.Request.HTML
};
